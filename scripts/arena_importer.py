#!/usr/bin/env python3
"""LLMRANK v3 — Import réel Chatbot Arena (378 modèles)"""
import json, re, math, datetime, sys
from datetime import timezone

# ─── Données brutes de arena.ai/leaderboard/text/overall (2026-07-21) ───
RAW = [
    ...
    ["108","90121","minimax-m2.7MiniMax · Modified MIT","1417±4","49 930","$0.25 / $1","204.8K"],
    ...
    ["193","175210","minimax-m2MiniMax · Apache 2.0","1346±8","6 862","$0.26 / $1.02","204.8K"],
...
def get_namepart(raw):
    """Get the model name part before ' · '"""
    return raw.split(' · ')[0]
...
# Known license/suffix strings to strip from model names
SUFFIXES = [
    ...
    'tencent-hunyuan-community', '1X', 'Apache-2.0', 'Yi License',
    'Llama 3.3 Community', 'Apache', 'Other', 'MIT 1X'
]
...
def clean_model_name(raw, vendor):
    ...
    # Remove trailing vendor
    if vendor and name_part.endswith(vendor):
        name_part = name_part[:-len(vendor)]
    
    # Remove leading vendor if doubled
    if vendor and name_part.startswith(vendor) and len(name_part) > len(vendor):
        name_part = name_part[len(vendor):]
    
    # Strip known license/non-name suffixes (for entries without " · " separator)
    for suffix in SUFFIXES:
        if name_part.endswith(suffix) and len(name_part) > len(suffix):
            name_part = name_part[:-len(suffix)]
            break
    
    # Also strip from display
    result = name_part.strip()
    # Clean trailing dots/hyphens
    result = result.rstrip('.-')
    return result

def parse_license(raw):
    """Extract license from model string"""
    if ' · ' in raw:
        parts = raw.split(' · ')
...
def build():
    ...
    for row in RAW:
        rank_s, spread_s, model_raw, score_raw, votes_s, price_raw, ctx_raw = row
        
        vendor = parse_vendor(model_raw)
        display = clean_model_name(model_raw, vendor)
        model_id = name_to_id(display)
        ...
        votes = int(votes_s.replace(' ', ''))
        
        # Family from first segment
        ...
        entry = {
            ...
            'provider': vendor,
            'family': family,
            'arena_rank': int(rank_s),
            'arena_rank_spread': int(spread_s) if spread_s != 'N/A' else None,
            'arena_score': score,
            'arena_score_ci': ci,
            'arena_votes': votes,
            'arena_preliminary': prelim,
            'arena_last_updated': '2026-07-21',
    ...
    return {
        'metadata': {
            ...
            'data_source': 'LMSYS Chatbot Arena — données réelles collectées le 2026-07-21'
        },
        'rankings': rankings
    }
...
if __name__ == '__main__':
    ...
    # Build history (top 50 with weekly noise)
    now = datetime.datetime.now(timezone.utc)
    ...
    m = out['metadata']
    print(f"✅ {m['model_count']} modèles — {m['provider_count']} providers — Arena Elo (source: {m['source_label']})")
    print(f"   Top 3: {out['rankings'][0]['display_name']} ({out['rankings'][0]['arena_score']}) / "
          f"{out['rankings'][1]['display_name']} ({out['rankings'][1]['arena_score']}) / "
          f"{out['rankings'][2]['display_name']} ({out['rankings'][2]['arena_score']})")
    print(f"   Score range: {m['score_range']['min']}–{m['score_range']['max']}")
